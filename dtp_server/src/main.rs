                ...
                if client.conn.is_established() {
                    if client.next_timeout.is_none() || client.next_timeout <= Some(Instant::now()) {
                        ...
                        for _ in client.dtp_config_offset..cfgs.len() {
                            ...
                            let mut len = 0;
                            unsafe {
                                len = match client.conn.stream_send_full(stream_id as u64, &DATA_BUF[0..block_size as usize], true, deadline, priority, depend_id) {
                                    Ok(l) => l,
                                    Err(err) => panic!("{:?}", err),
                                };
                            }
